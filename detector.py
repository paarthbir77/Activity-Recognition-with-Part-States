...
class DetectorTF2:
	...
	def crop_person(self, image, boxes_list):
		if not boxes_list:
			pass
		img = image.copy()
		persons = []
		for idx in range(len(boxes_list)):
			...
			# Cropping only person class
			if cls == "person":
				crop = img[y_min:y_max, x_min:x_max]
				persons.append(crop)
				#if save_output:
				#	cv2.imwrite("images/crops/"+str(filename)+str(idx)+".jpg", crop)
				# Display crop
				#cv2.imshow('detected object crop', crop)
				#cv2.waitKey(0)
		return persons

	def get_person_bbox(self, image, boxes_list):
		if not boxes_list:
			pass
		img = image.copy()
		person_bbox = []
		for idx in range(len(boxes_list)):
			...
			if cls == "person":
				crop = img[y_min:y_max, x_min:x_max]
				person_bbox.append( [(x_min, y_min), (x_max, y_max)] )
